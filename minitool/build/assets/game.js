...
(function () {
  ...
  /* 小红书离线版：容器无网络、禁外链——兑换码在网页完整版购买，此处仅保留离线验码解锁 */
  ...
  function paywallModal(t) {
    ...
    const html =
      ...
      '<div class="pw-qr-note">💡 小工具离线版无法跳转购买页：兑换码请到<b>网页完整版</b>购买（浏览器搜「SHUAMONE 耍门」即可），支持微信 / 支付宝，付款自动发码；拿到 NOVA-XXXXXX 后回到这里输入，本地即可解锁</div>' +
  ...
  function modalStats() {
    ...
    openModal('战报 · STATS', html +
      '<div style="margin-top:12px"><button class="retrybtn" id="lbBtn" style="margin:0">🔥 贡献榜</button></div>' +
  ...
  function renderPlantWidget() {
    ...
    if (!w) {
      ...
      css.textContent = '#nvPlant{position:fixed;left:14px;bottom:14px;z-index:800;cursor:pointer;background:rgba(10,15,26,.82);border:1px solid #26324a;border-radius:16px;padding:6px 8px 2px;-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);transition:border-color .15s,transform .15s;box-shadow:0 4px 14px rgba(0,0,0,.4)}#nvPlant:hover{border-color:#34d399;transform:translateY(-2px)}#nvPlant .np-l{display:block;font-size:9px;color:#8b9bb4;text-align:center;letter-spacing:2px;margin-top:2px}';
