...
(function () {
  ...
  /* 8 个预设头像（几何小生物，无刻板印象） */
  var AV = [
    { n: '机器人', s: '<svg viewBox="0 0 48 48"><rect x="8" y="14" width="32" height="26" rx="6" fill="#22d3ee"/><rect x="14" y="22" width="6" height="6" fill="#07131c"/><rect x="28" y="22" width="6" height="6" fill="#07131c"/><rect x="18" y="32" width="12" height="3" rx="1.5" fill="#07131c"/><line x1="24" y1="14" x2="24" y2="7" stroke="#22d3ee" stroke-width="3"/><circle cx="24" cy="6" r="3" fill="#fbbf24"/></svg>' },
    { n: '橘猫', s: '<svg viewBox="0 0 48 48"><circle cx="24" cy="27" r="17" fill="#fbbf24"/><polygon points="10,16 16,4 22,15" fill="#fbbf24"/><polygon points="38,16 32,4 26,15" fill="#fbbf24"/><circle cx="18" cy="25" r="2.6" fill="#31220b"/><circle cx="30" cy="25" r="2.6" fill="#31220b"/><polygon points="24,29 21,32 27,32" fill="#f97316"/><path d="M14 33 q10 7 20 0" stroke="#31220b" fill="none" stroke-width="2"/></svg>' },
    { n: '小幽灵', s: '<svg viewBox="0 0 48 48"><path d="M10 44 V22 a14 14 0 0 1 28 0 V44 l-4.6-4 -4.7 4 -4.7-4 -4.7 4 -4.7-4 z" fill="#a78bfa"/><circle cx="18" cy="22" r="3" fill="#241a3f"/><circle cx="30" cy="22" r="3" fill="#241a3f"/><circle cx="24" cy="30" r="2" fill="#241a3f" opacity=".6"/></svg>' },
    { n: '星球', s: '<svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="13" fill="#60a5fa"/><ellipse cx="24" cy="24" rx="21" ry="6" fill="none" stroke="#93c5fd" stroke-width="2.4"/><circle cx="19" cy="20" r="3" fill="#1e3a8a"/><circle cx="28" cy="27" r="2.2" fill="#1e3a8a"/></svg>' },
    ...
    { n: '蘑菇', s: '<svg viewBox="0 0 48 48"><path d="M8 24 a16 13 0 0 1 32 0 z" fill="#f87171"/><circle cx="17" cy="18" r="3" fill="#fecaca"/><circle cx="29" cy="16" r="2.4" fill="#fecaca"/><rect x="18" y="24" width="12" height="14" rx="5" fill="#fef3c7"/><circle cx="21" cy="30" r="1.4" fill="#d6d3d1"/><circle cx="27" cy="33" r="1.2" fill="#d6d3d1"/></svg>' },
    { n: '小鱼', s: '<svg viewBox="0 0 48 48"><polygon points="10,24 18,16 18,32" fill="#34d399"/><ellipse cx="28" cy="24" rx="14" ry="10" fill="#34d399"/><circle cx="34" cy="22" r="2.4" fill="#064e3b"/><path d="M28 14 q4 -5 6 0" stroke="#065f46" fill="none" stroke-width="2"/></svg>' }
  ];
  ...
  function html(game) {
    ...
    return '<style>' +
      ...
      '.fbw .fb-cols{display:flex;align-items:stretch}' +
      ...
      '.fb-card{display:flex;background:#fffdf6;border:1px solid #e2d8bf;border-radius:6px;padding:8px 10px;margin-bottom:8px;box-shadow:1.5px 2px 6px rgba(0,0,0,.12);transform:rotate(-.5deg)}' +
      ...
      '.fc-b{min-width:0;flex:1}.fc-h{display:flex;align-items:baseline;flex-wrap:wrap}.fc-h b{font-size:13px}.fc-h .fc-r{font-size:10px;color:#8a7f68;border:1px solid #c9bd9c;border-radius:99px;padding:0 7px}.fc-h .fc-g{font-size:10px;color:#3f6f8f}.fc-h i{font-size:10px;color:#b0a488;margin-left:auto;font-style:normal}' +
      ...
      '.fbw .fb-namerow{position:relative;display:flex;align-items:center;flex-wrap:wrap;margin-top:8px}' +
      ...
      '.fbw .fb-pop{display:none;position:absolute;top:38px;left:0;z-index:6;background:#fffdf6;border:1.5px solid #b9ac8d;border-radius:10px;padding:9px;box-shadow:0 8px 20px rgba(0,0,0,.22);width:224px}' +
      ...
      '.fbw .fb-anon{display:flex;align-items:center;font-size:12px;color:#6f6753;margin-top:8px;cursor:pointer;user-select:none}' +
      '.fbw .fb-anon input{accent-color:#c2452d}' +
      '.fbw .fb-foot{display:flex;align-items:center;margin-top:10px;flex-wrap:wrap}' +
      ...
      '.fbw .fb-cols > * + *{margin-left:14px}' +
      '.fb-card > * + *{margin-left:8px}' +
      '.fc-h > * + *{margin-left:6px}' +
      '.fbw .fb-namerow > * + *{margin-left:8px}' +
      '.fb-pop > * + *{margin-left:5px}' +
      '.fbw .fb-anon > * + *{margin-left:6px}' +
      '.fbw .fb-foot > * + *{margin-left:8px}' +
      '.fbw .fb-chip + .fb-chip{margin-left:5px}' +
      ...
      '<div style="display:flex;flex-wrap:wrap;margin-top:6px" id="fbRoles">' + ROLES.map(function (r) { return '<button type="button" class="fb-chip' + (d.role === r ? ' on' : '') + '" data-r="' + esc(r) + '">' + esc(r) + '</button>'; }).join('') + '</div>' +
      ...
      '<div class="fb-foot"><button type="button" class="fb-send" id="fbSend">📝 写进年鉴</button></div>' +
      '<div class="fb-hint">年鉴墙保存在你自己的浏览器里；离线版留言保存在本机；想同步给作者，可到网页完整版（浏览器搜「SHUAMONE 耍门」）留言。这个项目大部分免费、为爱发电——真话最重要。</div>' +
      '</div></div></div>';
  }
