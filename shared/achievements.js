...
window.ACHS = (function () {
  ...
  function render(target) {
    ...
    var cards = ACH.map(function (a) {
      ...
      return '<div style="display:flex;gap:10px;align-items:flex-start;border:1px solid ' + (on ? 'var(--line)' : '#141b28') +
        ';border-radius:10px;padding:10px 12px;' + (on ? 'background:linear-gradient(135deg,rgba(34,211,238,.06),transparent)' : 'opacity:.55') + '">' +
        '<div style="font-size:22px;line-height:1.2;filter:' + (on ? 'none' : 'grayscale(1)') + '">' + (on ? a.icon : '🔒') + '</div>' +
    ...
    var html = head + '<div style="display:grid;grid-template-columns:1fr;gap:8px;max-height:56vh;overflow:auto;padding-right:4px">' + cards + '</div>';
