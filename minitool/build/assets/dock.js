...
(function () {
  ...
  function mount(opts) {
    ...
    css.textContent = '#nvDock{position:fixed;right:14px;display:flex;flex-direction:column;z-index:800}' +
      '#nvDock{bottom:14px;bottom:var(--safe-area-inset-bottom, env(safe-area-inset-bottom, 14px))}' +
      '#nvDock button + button{margin-top:8px}' +
      '#nvDock button{width:52px;height:52px;border-radius:14px;border:1px solid #26324a;background:rgba(10,15,26,.86);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#8b9bb4;font-size:10px;font-family:inherit;transition:border-color .15s,color .15s,transform .15s;box-shadow:0 4px 14px rgba(0,0,0,.4)}' +
