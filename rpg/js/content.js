      ...
      rounds: [
        {
          ...
          options: [
            { t: '趁热打铁投放大：DAU 势头正好，买量把规模做大', ok: false, fx: { cost: 6, trust: -4 }, fb: 'Victor：好奇心流量本来就不留，买量等于花钱请人来看一眼再走。', why: '虚荣指标陷阱：DAU 可以被好奇心流量和买量虚高，不产生留存价值。' },
            { t: '首次任务引导：场景模板 + 示例问题，让新用户 5 分钟内完成第一个成功任务', ok: true, fx: { users: 4, trust: 3, morale: 2 }, fb: 'Maya：漂亮。完成首个任务的用户 7 日回访 41%，是未完成者的 7 倍——这才是杠杆点。', why: '激活的定义就是第一次成功任务。提升"完成率"直接放大留存基本盘。' },
            { t: '快速上线 5 个新功能，用新鲜感把用户留下来', ok: false, fx: { cost: 6, quality: -2 }, fb: 'Lin：连一个任务都没完成的用户，不会因为多了 5 个入口而留下。', why: '未激活用户的问题不是选择不够多，而是第一次成功还没发生。' },
            { t: '全面改版 UI，提升第一印象', ok: false, fx: { morale: -2 }, fb: 'Raj：用户不是嫌丑，是不知道拿它干什么。改版解决不了引导缺失。', why: '把体验问题误诊为审美问题——线索"不知道能问什么"指向引导而非界面。' }
          ],
          hints: [
        ...
        },
        {
          ...
          options: [
            { t: 'DAU——投资人最认这个数', ok: false, fx: { trust: -2 }, fb: 'Victor：我今天刚被这个数骗过一次，你想让我再被骗一次？', why: 'DAU 可被好奇心流量虚高，不反映价值交付。' },
            { t: '累计注册用户数', ok: false, fx: {}, fb: 'Maya：累计数只增不减，讲的是历史不是价值。', why: '典型虚荣指标（vanity metric）。' },
            { t: '每周成功任务数（周活 × 人均成功任务）', ok: true, fx: { trust: 4 }, fb: 'Lin：对。AI 产品的价值就是"帮人完成任务"。这个数涨，公司才真的在前进。', why: 'AI 工具产品的价值锚点是任务完成，北极星应能拆解到杠杆动作。' },
            { t: 'API 调用总量', ok: false, fx: {}, fb: 'Kai：调用量会被重试和脚本刷高，而且调用了不等于成功了。', why: '调用量是过程量，不等于价值量。' }
          ],
      ...
      rounds: [
        {
          ...
          options: [
            ...
            { t: '先不上线，离线再跑一轮评测', ok: false, fx: {}, fb: 'Kai：同样的数据再跑十遍还是这个分，问题根本不在"跑几遍"。', why: '重复评测不改变泄漏与分布差——离线验证不了在线问题。' },
      ...
      rounds: [
        {
          ...
          options: [
            ...
            { t: '加个等待时间倒计时', ok: false, fx: {}, fb: 'Kai：倒计时 8 秒？那是在提醒用户有多慢。', why: '同上——感知优化要做的是"更早给出价值"，不是"更精确地预告痛苦"。' }
          ],
    ...
    /* ============ CASE 6 ============ */
    {
      ...
      rounds: [
        ...
        },
        {
          ...
          options: [
            { t: '它压缩了模型权重，所以更快', ok: false, fx: {}, fb: 'Lin：压缩权重的是量化。Prefix Cache 是另一回事。', why: '混淆两个概念：量化压权重，缓存复用计算。' },
            ...
            { t: '它把用户的旧对话记住了', ok: false, fx: {}, fb: 'Lin：那是 Memory，和推理缓存是两码事。', why: '缓存（性能）≠ 记忆（能力）。' },
            { t: '它让模型变得更聪明', ok: false, fx: {}, fb: 'Lin：它不改变任何能力，只改变效率。', why: '缓存是性能优化，不动模型能力。' }
          ],
      ...
      rounds: [
        {
          ...
          options: [
            ...
            { t: 'RAG：检索 JSON 示例给模型看', ok: false, fx: {}, fb: 'Lin：检索解决"知识从哪来"，不解决"输出稳不稳"。', why: '工具错位：RAG 管知识供给，不管输出格式纪律。' },
        ...
        },
        {
          ...
          options: [
            ...
            { t: '让运营在后台手动维护 FAQ 匹配规则', ok: false, fx: {}, fb: '运营：你在让我人肉做搜索引擎？', why: '规则维护不可扩展，语义匹配能力差。' }
          ],
        ...
        },
        {
          ...
          options: [
            ...
            { t: 'RAG：检索品牌文案示例供参考', ok: false, fx: {}, fb: 'Lin：检索到示例只是"参考"，模型未必照做——语气不是知识，是行为。', why: 'RAG 供给知识，不保证行为一致。' },
            { t: 'Prompt：详细描述语气要求', ok: false, fx: {}, fb: 'Lin：能缓解，但长 Prompt 下风格漂移明显，且占 token。作为第一步可以，作为最终方案不稳。（本题要的是"根治"）', why: 'Prompt 是软约束，一致性有限——但注意：它是成本最低的第一步，实践中先 Prompt 验证再 SFT。' },
    ...
    /* ============ CASE 10 ============ */
    {
      ...
      rounds: [
        ...
        },
        {
          ...
          options: [
            { t: '给 prompt 加一条"永远不要用代码块包裹 JSON"——立了规矩就不会再犯', ok: false, fx: {}, fb: 'Kai：上次的教训还不够吗——Prompt 是软约束，风格漂移随时可能再触发。', why: '软约束不构成根治；同类故障仍会以新形态复发。' },
      ...
      rounds: [
        {
          ...
          options: [
            ...
            { t: '工程手滑改错了 prompt', ok: false, fx: {}, fb: 'Victor：个人的手滑是表象。如果制度能让一次手滑放行到生产，那就是制度的问题。', why: '把系统性问题归因于个人，无法防复发。' },
        ...
        },
        {
          ...
          options: [
            { t: '汇报做了多少事：建了多少用例、开了多少会', ok: false, fx: {}, fb: 'Victor：苦劳我不付钱。我要看变化。', why: '过程汇报≠价值证明。' },
            ...
            { t: '说团队这三个月非常努力', ok: false, fx: {}, fb: 'Victor：我知道他们努力。我問的是效果。', why: '情绪叙事不是商业论证。' }
          ],
